...
use crate::smart_id::errors::Exception::{DocumentUnusableException, InvalidParametersException, RequiredInteractionNotSupportedByAppException, SessionInProgress, SessionStatusMissingResult, SessionTimeoutException, TechnicalErrorException, UserRefusedCertChoiceException, UserRefusedConfirmationMessageException, UserRefusedConfirmationMessageWithVcChoiceException, UserRefusedDisplayTextAndPinException, UserRefusedException, UserRefusedVcChoiceException, UserSelectedWrongVerificationCodeException};
use crate::smart_id::models::{AuthenticationHash, AuthenticationSessionRequest, AuthenticationSessionResponse, HashType, Interaction, SemanticsIdentifier, SessionEndResultCode, SessionStatus, SessionStatusCode, SessionStatusRequest, SignableData, SmartIdAuthenticationResponse};
...
impl SmartIdClient {
    ...
    fn validate_session_status_result(&self, session_status: SessionStatus) -> Result<SessionStatus, Exception> {
            ...
            if let Some(result) = session_status.result {
    ...
    pub async fn get_session_status(&self,session_id:String) -> Result<SessionStatus, Exception> {
     ...
     async fn get_session_status_request(
        ...
        let url = format!("{}/session/{}", self.host_url, request.session_id);
        let response = self
            .client
            .get(url)
            .send()
